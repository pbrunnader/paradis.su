...
-spec encrypt(Password :: string(), Plain :: binary()) ->
		     Code :: binary().

-spec decrypt(Password::string(), Code::binary()) ->
		     Plain::binary().
...
rand_password() -> binary_to_list(crypto:rand_bytes(10)).


decrypt(Password, <<IV:16/binary,Bin1/binary>>) when is_list(Password) ->
...
encrypt(Password, Bin) when is_list(Password), is_binary(Bin) ->
    P = erlang:md5(Password),
